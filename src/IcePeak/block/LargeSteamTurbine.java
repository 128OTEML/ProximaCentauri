...
public class LargeSteamTurbine extends ConsumeGenerator {
    ...
    public LargeSteamTurbine(String name) {
        ...
        // 8倍效率，消耗也相应增加
        consumeLiquid(IcePeakLiquids.steam, 5f); // 0.625f * 8
    ...
    public void setBars() {
        ...
        addBar("steam", (LargeSteamTurbineBuild entity) -> new Bar(
            () -> "Steam: " + (int)entity.liquids.get(IcePeakLiquids.steam) + "/" + liquidCapacity,
            () -> Color.gray, // 使用灰色作为替代
            () -> entity.liquids.get(IcePeakLiquids.steam) / liquidCapacity
        ));
    }
    ...
    public class LargeSteamTurbineBuild extends ConsumeGeneratorBuild {
        ...
        public void updateTile() {
            ...
            if(liquids.get(IcePeakLiquids.steam) > 0 && Mathf.chance(effectChance * delta())) {
                // 绘制蒸汽效果
