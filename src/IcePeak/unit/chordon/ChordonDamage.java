...
import IcePeak.FlameOutSFX;
import IcePeak.effects.SpecialDeathEffects;
import IcePeak.special.SpecialMain;
import arc.Events;
...
public class ChordonDamage{
    private final static Seq<AbsoluteDamage<?>> damages = new Seq<>();
    private final static IntMap<AbsoluteDamage<?>> damageMap = new IntMap<>();
    private final static Seq<chordonHolder> units = new Seq<>();
    private final static IntMap<chordonHolder> chordonMap = new IntMap<>();
    ...
    private final static Seq<ChordonDeath> deaths = new Seq<>();
    ...
    public static void exclude(Unit unit){
        if(!SpecialMain.validChordonSpawn()) return;
    ...
    static void clearExclude(){
        ...
        if(!elem.getClassName().equals("flame.unit.chordon.chordonUnit")) return;
    ...
    static void chordonDeath(float x, float y, float rotation){
        ChordonDeath d = new ChordonDeath();
        d.x = x;
        d.y = y;
        d.rotation = rotation;
    ...
    public static void draw(){
        if(!deaths.isEmpty()){
            for(ChordonDeath d : deaths){
                d.draw();
            }
        }

        for(chordonHolder u : units){
            if(u.removeCount >= 5){
                u.unit.draw();
            }
        }
    ...
    public static void update(){
        if(!deaths.isEmpty()){
            deaths.removeAll(d -> {
                d.update();
                return d.time >= ChordonDeath.duration;
            });
        }
            ...
            for(chordonHolder u : units){
                if(!u.added){
                    ...
                    switch(u.removeCount){
                        ...
                        case 3, 4 -> {
                            activeAdd = false;
                            ChordonUnit en = u.unit;
                            chordonMap.remove(u.unit.id);
                            chordonMap.put(en.id, u);
                            u.unit = en;
                            activeAdd = true;
                        }
                    }
    ...
    static void addchordon(ChordonUnit unit){
        if(activeAdd && !chordonMap.containsKey(unit.id)){
            chordonHolder h = new chordonHolder();
            h.unit = unit;
            units.add(h);
            chordonMap.put(unit.id, h);
        }
    }
    static void removechordon(ChordonUnit unit){
        if(!activeAdd) return;

        chordonHolder eh = units.find(u -> u.unit == unit);
        if(eh != null && eh.time > 60f * 60f){
            chordonDeath(unit.x, unit.y, unit.rotation);

            units.remove(h -> h.unit == unit);
            chordonMap.remove(unit.id);
        }else if(eh != null){
            unit.duplicate();
        }
    }
    static void onDuplicate(ChordonUnit last, ChordonUnit latest){
        if(!activeAdd) return;
        chordonHolder h = chordonMap.get(last.id);
        if(h == null) h = new chordonHolder();
        chordonMap.remove(last.id);
        h.unit = latest;
        chordonMap.put(latest.id, h);
    }
    ...
    public static void damageUnitKnockback(Unit unit, float damage, float vx, float vy, Runnable onDeath){
        if(exclude.contains(unit.id) || chordonMap.containsKey(unit.id)){
    ...
    static class chordonHolder{
        private ChordonUnit unit;
    ...
    static class UnitAbsoluteDamage extends AbsoluteDamage<Unit>{
        ...
        void updateNaN(){
            ...
            for(WeaponMount m : u.mounts){
                if(isNaN |= ChordonDamage.isNaNInfinite(m.rotation, m.targetRotation, m.aimX, m.aimY)){
