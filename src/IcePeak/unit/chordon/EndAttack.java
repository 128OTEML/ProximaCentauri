...
public class EndAttack extends AttackAI{
    ...
    void update(){
        ...
        if(time < duration){
            ...
            for(int i = 0; i < 3; i++){
                TextureRegion reg = ChordonRegions.endAPI[i];
                for(int j = 0; j < 8; j++){
                    //endFlash
        ...
        if(endScanTimer <= 0f && endTime < (1.75f * 60f)){
            for(Unit u : Groups.unit){
                if(u.team != unit.team && unit.within(u, size)){
                    ChordonDamage.damageUnit(u, u.maxHealth + 1000f, true, () -> FlameFX.endDeath.at(u.x, u.y, u.hitSize / 2));
                }
            }
            tmpBuildings.clear();
            ...
            for(Building b : tmpBuildings){
                ChordonDamage.damageBuilding(b, b.maxHealth + 1000f, true, () -> FlameFX.endDeath.at(b.x, b.y, b.hitSize() / 2));
            }
                ...
                if(ringReloads[i] <= 0f){
                    ...
                    if(tar != null){
                        if(tar instanceof Unit u){
                            ...
                            ChordonDamage.damageUnit(u, 10000f + d, true, null);
                        }else if(tar instanceof Building b){
                            float d = Mathf.pow(b.health / 100000000f, 2) * b.maxHealth;
                            ChordonDamage.damageBuilding(b, 10000f + d, true, null);
                        }
    ...
    void drawEnd(){
        ...
        Draw.color(FlamePal.chordonDark);

        Draw.z(Layer.flyingUnit);
        float[] ar = crosses.items;
        int s = crosses.size;
        TextureRegion flash = ChordonRegions.flash;
    ...
    void draw(){
        if(ending) drawEnd();
        //Draw.color(FlamePal.chordonAdd);
        Color darkC = Tmp.c1.set(FlamePal.chordonDark).lerp(FlamePal.red, Mathf.absin(2f, 1f));
        Color col1 = Tmp.c2.set(FlamePal.chordon).lerp(FlamePal.red, Mathf.absin(2f, 1f));

        float fin = Mathf.clamp(time / 40f);
        Draw.blend(Blending.additive);
        Draw.color(darkC, fin * 0.5f);
        
        TextureRegion mag = ChordonRegions.magicCircle;
