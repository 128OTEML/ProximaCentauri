...
public class ProximaSFX implements ApplicationListener{
    public static ProximaSFX inst;
    ...
    void draw(){
        ...
        if(!blackHoleQueue.isEmpty()){
            ProximaShaders.blackholeShader.holes.addAll(blackHoleQueue);
            Draw.draw(Layer.floor - 1f, () -> buffer.begin(Color.clear));
            Draw.draw(Layer.blockOver + 0.1f, () -> {
                buffer.end();
                buffer.blit(ProximaShaders.blackholeShader);
                ProximaShaders.blackholeShader.holes.clear();
            });
        ...
        if(impFrameTime > 0f && !impFrameEntities.isEmpty()){
            Draw.draw(Layer.end - 1, () -> {
                ...
                ProximaShaders.harshShadow.clear();
                for(ImpactFrameDrawer impact : impFrameEntities){
                    float x = impact.x;
                    float y = impact.y;
                    ProximaShaders.harshShadow.addLight(x, y, impact.intensity);
                ...
                Draw.shader(ProximaShaders.alphaCut);
                ...
                buffer.end();
                buffer.blit(ProximaShaders.harshShadow);

                //ProximaShaders.harshShadow.addLight(800, 600, 12f);
                //ProximaShaders.impactShader.addLight(player.x + 20f, player.y + 110f, 5f);
                //ProximaShaders.impactShader.addLight(player.x, player.y, 20f);
    ...
    public void drawOrderPortal(float x, float y, float width, float height){
        Draw.draw(Layer.flyingUnitLow - 0.5f, () -> {
            ProximaShaders.orderShader.width = width;
            ProximaShaders.orderShader.height = height;
            ProximaShaders.orderShader.srcX = x;
            ProximaShaders.orderShader.srcY = y;

            Draw.shader(ProximaShaders.orderShader);
            Draw.shader();
        });
    }
    public void drawChaosPortal(float x, float y, float width, float height){
        Draw.draw(Layer.flyingUnitLow - 0.5f, () -> {
            ProximaShaders.chaosShader.width = width;
            ProximaShaders.chaosShader.height = height;
            ProximaShaders.chaosShader.srcX = x;
            ProximaShaders.chaosShader.srcY = y;

            //Draw.flush();
            Draw.shader(ProximaShaders.chaosShader);
            //Draw.flush();
            Draw.shader();
        });
    }
