...
public class ProtectedDeath{
    ...
    void draw(){
        ...
        if(interf > 0){
            ...
            for(TextureRegion tr : ProtectedRegions.endAPI){
                float c = 0.4f;
                float c1 = count / 2f;
                float cf = pow2.apply(Mathf.curve(interf, c * c1, (1f - c) + c * c1));
                Draw.color(ProximaPal.red, cf * fout);
        ...
        if(chains > 0 && chains < 1){
            ...
            for(int i = 0; i < 16; i++){
                ...
                Tmp.c1.set(ProximaPal.red).a(1f - Mathf.curve(chains, 0.5f, 1f));
