...
public class ProtectedDamage{
    ...
    private static ProtectedSpawner spawner = null;
    private final static Seq<ProtectedDeath> deaths = new Seq<>();
    ...
    public static void reset(){
        ...
        if(spawner != null){
            ...
            if(!units.isEmpty()){
                ProtectedUnit u = units.first().unit;
    ...
    static void empathyDeath(float x, float y, float rotation){
        ProtectedDeath d = new ProtectedDeath();
        d.x = x;
        d.y = y;
        d.rotation = rotation;
    ...
    public static void spawnEmpathy(float x, float y){
        if(spawner == null && SpecialMain.validEmpathySpawn()){
            ProtectedSpawner s = new ProtectedSpawner();
            s.x = x;
            s.y = y;
            s.active = true;
            s.shouldSpawn = true;
    ...
    public static void update(){
        ...
        if(!deaths.isEmpty()){
            deaths.removeAll(d -> {
                d.update();
                return d.time >= ProtectedDeath.duration;
            });
        }
            ...
            for(EmpathyHolder u : units){
                if(!u.added){
                    ...
                    switch(u.removeCount){
                        ...
                        case 3, 4 -> {
                            activeAdd = false;
                            ProtectedUnit en = u.unit.duplicate();
    ...
    static void addEmpathy(ProtectedUnit unit){
    ...
    static void removeEmpathy(ProtectedUnit unit){
        if(!activeAdd) return;
    ...
    static void onDuplicate(ProtectedUnit last, ProtectedUnit latest){
        if(!activeAdd) return;
        ...
        void setDefaultDeath(){
            lastRunnable = () -> {
                if(entity instanceof Unit u){
                    SpecialDeathEffects eff = SpecialDeathEffects.get(String.valueOf(u.type));
                    ...
                    SpecialDeathEffects eff = SpecialDeathEffects.get(String.valueOf(b.block));
    ...
    static class UnitAbsoluteDamage extends AbsoluteDamage<Unit>{
        ...
        void updateNaN(){
            ...
            for(WeaponMount m : u.mounts){
                if(isNaN |= ProtectedDamage.isNaNInfinite(m.rotation, m.targetRotation, m.aimX, m.aimY)){
