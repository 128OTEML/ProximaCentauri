...
public class ProtectedUnit extends UnitEntity{
    Seq<ProtectedAI> movementAIs = new Seq<>(), attackAIs = new Seq<>();
    ProtectedAI activeMovement, activeAttack;
    ...
    private final static WeightedRandom<ProtectedAI> randAI = new WeightedRandom<>();
    private final static float chainLifetime = 17f;
    private final static Rect scanRect = new Rect();

    static ProtectedUnit createUnit(){
        return createUnit(null);
    }

    static ProtectedUnit createUnit(EmpathyTransferData last){
        ...
        return new ProtectedUnit(){
            ...
            void updateDamageTaken(float amount){
                ...
                if(d[idecoyDelay] <= 0f && (d[idamageTaken] >= 700000f || ProtectedDamage.isNaNInfinite(d[idamageTaken], d[imaxDmgTaken]))){
                    duplicate();
                    d[imaxDmgTaken] = 0f;
                    d[idmgDelay] = 0f;
                }
            ...
            public void heal(float amount){
                if(ProtectedDamage.isNaNInfinite(amount)) amount = 0f;
            ...
            public void add(){
                if(!added && !initialized){
                    ...
                    // 确保列表不为空
                    if(movementAIs.isEmpty()){
                        Log.warn("movementAIs is empty, adding default");
                        movementAIs.add(new BasicMovementAI().set(this));
                    }
                    if(attackAIs.isEmpty()){
                        Log.warn("attackAIs is empty, adding default");
                        attackAIs.add(new BasicAttackAI().set(this));
                    }

                    // 先设置 activeMovement 和 activeAttack 为第一个 AI
                    activeMovement = movementAIs.first();
                    activeAttack = attackAIs.first();

                    // 再调用 randAI 可能会覆盖，但至少不为 null
                    randAI(true, false);
                    randAI(false, false);

                    // 再次确认不为 null
                    if(activeAttack == null && !attackAIs.isEmpty()){
                        activeAttack = attackAIs.first();
                    }
                    if(activeMovement == null && !movementAIs.isEmpty()){
                        activeMovement = movementAIs.first();
                    }

                    if(activeAttack != null && activeMovement != null){
                        activeAttack.init();
                        activeMovement.init();
                    }
                    ...
                    ProtectedDamage.addEmpathy(this);

                    Log.info("ProtectedUnit: add completed - activeAttack=@, activeMovement=@",
                            activeAttack != null, activeMovement != null);
                }
                super.add();
            }
            ...
            public void remove(){
                if(d[ihealth] <= 0f || d2[0]){
                    if(!d2[0]){
                        ProtectedDamage.removeEmpathy(this);
                    }
                    for(ProtectedAI ai : attackAIs){
                        ai.stopSounds();
                    }
                    for(ProtectedAI ai : movementAIs){
                        ai.stopSounds();
                    }
                    super.remove();
                }
            ...
            ProtectedUnit duplicate(){
                ...
                u1.add();
                ProtectedDamage.onDuplicate(this, u1);
    ...
    public void update(){
        ...
        // 确保 AI 不为 null，如果为 null 则重新初始化
        if(activeAttack == null && !attackAIs.isEmpty()){
            Log.info("ProtectedUnit: activeAttack is null, reinitializing...");
        ...
        if(activeMovement == null && !movementAIs.isEmpty()){
            Log.info("ProtectedUnit: activeMovement is null, reinitializing...");
        ...
        // 如果仍然为 null，创建临时 AI 防止崩溃
        if(activeAttack == null){
            Log.warn("ProtectedUnit: Creating temporary attack AI");
            BasicAttackAI tempAI = new BasicAttackAI();
            tempAI.set(this);
            tempAI.attack = true;
            attackAIs.add(tempAI);
            activeAttack = tempAI;
            activeAttack.init();
        }

        if(activeMovement == null){
            Log.warn("ProtectedUnit: Creating temporary movement AI");
            BasicMovementAI tempAI = new BasicMovementAI();
            tempAI.set(this);
            tempAI.attack = false;
            movementAIs.add(tempAI);
            activeMovement = tempAI;
            activeMovement.init();
        }
        // ========== 兜底初始化结束 ==========

        float lastDelta = Time.delta;
        ProximaSFX.realDelta = Time.delta;
        Time.delta = ProximaSFX.timeDelta;
        ProximaSFX.realDelta = Time.delta;
        Time.delta = ProximaSFX.timeDelta;
        ...
        if(chainTime > 0){
            ...
            if(chainTime <= 0f && getTarget() instanceof Unit u){
                ...
                ProtectedDamage.nanLock(u, pos.x, pos.y);
                chainTime = -240f;
            }
        }
        ...
        if(getTarget() instanceof Unit u && chainTime <= 0f && !isDecoy()){
            ...
            if(chainTime > -1){
                boolean nan;
                if((nan = ProtectedDamage.isNaNInfinite(u.x, u.y, u.rotation)) || !Tmp.r1.contains(u.x, u.y)){
        ...
        if(stunTimer <= 0f){
            if(activeAttack != null && activeMovement != null){
                ...
                Log.warn("ProtectedUnit: AI still null in update! attack=@, movement=@",
                        activeAttack != null, activeMovement != null);
            }
        }
    ...
    void updateDamageTaken(float amount){
        ...
        //1000000f
        if(decoyDelay <= 0f && (damageTaken >= 700000f || ProtectedDamage.isNaNInfinite(damageTaken, maxDamageTaken))){
            duplicate();
            damageTaken = 0f;
            maxDamageTaken = 0f;
        }
...
//添加攻击AI的位置
    ...
    private void updateNearby(){
        ...
        if(nearbyScan <= 0f){
            ...
            if(ProtectedDamage.isNaNInfinite(nearestTotalHealth)){
    ...
    */
    ...
    public void draw(){
        ...
        if(!isDecoy()){
            ...
            Draw.color();
            Draw.rect(ProtectedRegions.decoy, x, y, rotation - 90f);
        }
        ...
        if(chainTime > 0){
            ...
            Tmp.c1.set(Color.white).lerp(ProximaPal.empathyAdd, Mathf.curve(fin, 0f, 0.5f));
    ...
    void parry(boolean effect){
        ...
        // 添加空值保护，避免 ProximaSFX.inst 为 null
        ProximaSFX sfx = ProximaSFX.inst;
        boolean hasSFX = sfx != null;

        Utils.scanCone((QuadTree<Bullet>)Groups.bullet.tree(), x, y, rotation, 35f, 80f, b -> {
            if(b.team != team){
                // 添加空值保护
                float sr = hasSFX ? sfx.getBulletDps(b.type) : 0f;
                if(sr > scr){
                    strb = b;
                }
        ...
        for(LaserBulletHolder pr : parryLasers){
            // 添加空值保护
            float sr = hasSFX ? sfx.getBulletDps(pr.type) : 0f;
            if(sr > scr){
                strP = pr;
                strb = null;
            }
        ...
        if(parryHitSuccess){
            if(effect){
                ProximaFX.empathyParry.at(x, y, rotation);
                ProximaSounds.empathyParry.at(x, y, Mathf.random(0.99f, 1.01f));
            }
    ...
    */
    ...
    void initAIs(){
        ...
        movementAIs.addAll(
                new BasicMovementAI().set(this)
        );

        attackAIs.addAll(
                new BasicAttackAI().set(this)
        );

        Log.info("ProtectedUnit: initAIs completed - movement: @, attack: @", movementAIs.size, attackAIs.size);
    }
    ...
    void initDecoyAIs(){
        ...
        Log.info("ProtectedUnit: initDecoyAIs completed - movement: @, attack: @", movementAIs.size, attackAIs.size);
    }
    ...
    */
    ...
    void copyFields(ProtectedUnit f){
        x = tx = f.tx;
        y = ty = f.ty;
        ...
        for(ProtectedAI ai : f.attackAIs){
            ai.set(this);
            attackAIs.add(ai);
        }
        for(ProtectedAI ai : f.movementAIs){
            ai.set(this);
            movementAIs.add(ai);
        }
    ...
    ProtectedUnit duplicate(){
        ProtectedUnit u = new ProtectedUnit();
        u.team = team;
        u.setType(type);
        u.elevation = 1f;
        u.copyFields(this);
        ...
        u.add();
        ProtectedDamage.onDuplicate(this, u);
    ...
    void updateUsages(boolean attack){
        Seq<ProtectedAI> ais = attack ? attackAIs : movementAIs;
        int c = 0;
        for(ProtectedAI ai : ais){
            if(ai.weight() > 0) c++;
        }
        for(ProtectedAI ai : ais){
    ...
    float getTargetHealthFract(){
        ...
        float health = ProtectedDamage.getHealth(getTarget());
        ...
        if(attack){
            ...
            if(i == null){
                if(attackAIs.size > 0){
                    ...
                    Log.warn("ProtectedUnit: No attack AI available");
                    return;
                }
            }
            ...
            // 空值保护
            if(i == null){
                if(movementAIs.size > 0){
                    i = movementAIs.first();
                } else {
                    Log.warn("ProtectedUnit: No movement AI available");
                    return;
                }
            }
    ...
    void switchAI(ProtectedAI ai){
        if(ai == null) return;  // 添加空值保护
