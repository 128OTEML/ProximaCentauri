...
public class ProximaFX{
    public static Effect shield = new Effect(30f, e -> {
        blend(Blending.additive);
        color(Tmp.c1.set(ProximaPal.primary).a(Mathf.absin(e.fin(pow2Out), 1f / 50f, 1f) * 0.5f * e.fout()));
    ...
    aoeExplosion2 = new Effect(80f, 500f, e -> {
        ...
        z(z);
        color(ProximaPal.primary, Pal.lightOrange, Color.gray, e.fin());
    ...
    apathyCrit = new Effect(80f, e -> {
        ...
        for(int i = 0; i < 45; i++){
            ...
            color(ProximaPal.primary, ProximaPal.blood, pow2.apply(time));
    ...
    bigLaserHitSpark = new Effect(15f, e -> {
        color(Color.white, ProximaPal.primary, e.fin());
        ...
        for(int i = 0; i < 6; i++){
            ...
            if(f > 0.001f){
                color(Color.white, ProximaPal.primary, f);
    ...
    bigLaserHit = new Effect(30f, e -> {
        color(Color.white, ProximaPal.primary, Color.gray, pow2Out.apply(e.fin()));
    ...
    destroySparks = new Effect(40f, 1200f, e -> {
        ...
        for(int i = 0; i < isize2; i++){
            ...
            if(f < 1){
                color(ProximaPal.paleYellow, Pal.lightOrange, Color.gray, f);
        ...
        for(int i = 0; i < isize; i++){
            ...
            if(f < 1){
                color(ProximaPal.paleYellow, Pal.lightOrange, Color.gray, pow2In.apply(f));
                Lines.lineAngle(v.x + e.x, v.y + e.y, v.angle(), (size / 3f) * rsize * (1 - f));
            }
        }

        color(ProximaPal.paleYellow);
        for(int i = 0; i < 4; i++){
            float rot = i * 90f;
    ...
    empathyParryExplosion = new Effect(40f, e -> {
        color(ProximaPal.empathyDark, e.fout());
        blend(Blending.additive);
    ...
    empathyPrimeStrike = new Effect(40f, 300f, e -> {
        ...
        Tmp.c1.set(ProximaPal.empathyAdd).a(Mathf.clamp((e.lifetime - e.time) / 30f));
        color(Tmp.c1);
        blend(Blending.additive);
    ...
    empathyLightningHit = new Effect(14f, e -> {
        ...
        color(Color.white, ProximaPal.empathy, e.fin());
    ...
    empathyBlast = new Effect(60f, 900f, e -> {
        Draw.color(Color.white, ProximaPal.empathyAdd, pow2Out.apply(e.fin()));
    ...
    empathyBigLaserHit = new Effect(30f, e -> {
        color(Color.white, ProximaPal.empathy, Color.gray, pow2Out.apply(e.fin()));
    ...
    empathyDepowered = new Effect(40f, 1200f, e -> {
        ...
        color(ProximaPal.red, ProximaPal.empathyAdd, e.fin());
        blend(Blending.additive);
    ...
    endDeath = new Effect(50f, 1000f, e -> {
        ...
        Draw.color(ProximaPal.darkRed, ProximaPal.empathy, Mathf.curve(pow2Out.apply(fin1), 0f, 0.5f));
        ...
        Draw.color(ProximaPal.darkRed, ProximaPal.empathy, Mathf.curve(pow2Out.apply(e.fin()), 0f, 0.5f));
    ...
    coloredHit = new Effect(15f, e -> {
        ...
        color(Color.white, ProximaPal.red, e.fin());
        Lines.stroke(0.5f + e.fout());
    ...
    desGroundHit = new Effect(30f, 250f, e -> {
        ...
        color(ProximaPal.melt, e.color, pow3Out.apply(e.fin()));
    ...
    desNukeShoot = new Effect(35f, e -> {
        ...
        for(int i = 0; i < 5; i++){
            for(int j = 0; j < 7; j++){
                ...
                Draw.color(ProximaPal.red, Color.white, f);
    ...
    desMissileHit = new Effect(50f, 800f, e -> {
        ...
        for(int i = 0; i < 4; i++){
            ...
            color(Tmp.c2, ProximaPal.red, i / 3f);
            for(int j = 0; j < 5; j++){
                for(int k = 0; k < 9; k++){
                    float f = Mathf.curve(e.fin(), 0f, 1f - r.random(0.3f));
                    float rot = e.rotation + r.range(ang);
